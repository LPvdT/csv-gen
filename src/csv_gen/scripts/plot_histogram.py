...
t_min = (
    float(args.t_min) if args.t_min else np.min(list(map(np.min, all_times)))
)
t_max = (
    float(args.t_max) if args.t_max else np.max(list(map(np.max, all_times)))
)
...
plt.legend(
    ...
    prop={
        "size": 10,
        "family": ["Source Code Pro", "Fira Mono", "Courier New"],
    },
)
